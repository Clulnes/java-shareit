...
public class BookingController {
	...
	public ResponseEntity<Object> approve(@RequestHeader(Constants.USER_ID_HEADER) long userId,
		...
		log.info("Подтверждение бронирования {}, status={}, userId={}", bookingId, approved, userId);
		return bookingClient.approveBooking(userId, bookingId, approved);
	}

	@GetMapping("/owner")
	public ResponseEntity<Object> getBookingsByOwner(@RequestHeader(Constants.USER_ID_HEADER) long userId,
										@RequestParam(name = "state", defaultValue = "ALL") String stateParam,
										@PositiveOrZero @RequestParam(name = "from", defaultValue = "0") Integer from,
										@Positive @RequestParam(name = "size", defaultValue = "10") Integer size) {
		BookingState state = BookingState.from(stateParam)
				.orElseThrow(() -> new IllegalArgumentException("Unknown state: " + stateParam));
		log.info("Запрос бронирований владельца со state {}, userId={}, from={}, size={}", stateParam, userId, from, size);
